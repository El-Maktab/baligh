...
This module wires the four preprocessing stages into a single callable:
...
The public entry point is :func:`preprocess`.
...
def preprocess(input: PreprocessingInput) -> PreprocessingOutput:
    ...
    Chains the four stages in order:
        ...
        2. Word Boundary Detection — splits completed prefix from the active
           fragment being typed and determines NWP vs WAC mode.
        3. Segmentation — Farasa segments completed tokens, deriving
    ...
    # ------------------------------------------------------------------ #
    # Stage 3 — Segmentation (completed prefix only)                       #
    # ------------------------------------------------------------------ #
    tokens = segment(completed_prefix, norm_to_orig_map)

    # ------------------------------------------------------------------ #
    # Stage 4 — Morphological Analysis + Disambiguation                    #
    # ------------------------------------------------------------------ #
