"""CRF wrapper for HuggingFace token classification models."""
...
class BertCRFForTokenClassification(nn.Module):
    ...
    def __init__(
        self, base_model: nn.Module, num_labels: int, label2id: dict | None = None
    ):
        """Initialize BertCRFForTokenClassification with a base model and CRF layer."""
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            if max_len == 0:
                ...
                return {
                    "logits": torch.empty(
                        (0, 0), dtype=torch.long, device=emissions.device
                    )
                }
            padded = torch.full(
                (len(decoded), max_len), -100, dtype=torch.long, device=emissions.device
            )
            for i, seq in enumerate(decoded):
                padded[i, : len(seq)] = torch.tensor(
