...
class SubwordProjector:
    ...
    def __init__(self):
        """Initialize SubwordProjector with an AraBERT tokenizer."""
    ...
    def _tokenize_words(self, words: list[str]) -> list[list[str]]:
        ...
        if len(words) == 0:
            return []
        return [
            self.tokenizer.tokenize(word) if word != " " else [" "] for word in words
        ]
    ...
    def project(
        ...
        for word_tokens, word_edits in zip(segments, edits_list, strict=False):
            token_projection: list[list[Alignment]] = [[] for _ in word_tokens]
    ...
    def flatten(self, projected_examples: ProjectedExample) -> ProjectedExample:
        """Flatten nested subword lists into a single list."""
