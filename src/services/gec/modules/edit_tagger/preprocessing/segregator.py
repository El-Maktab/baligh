...
class Segregator:
    ...
    def segregate(
        self, proj_examples: list[ProjectedExample]
    ) -> tuple[ProjectedExample, ProjectedExample]:
        ...
        for example in proj_examples:
            for token, label in zip(example.subwords, example.labels_star):
                if self._is_punctuation_edit(token, label):
                    ...
                    coun2 += 1
        print(count)
        punc_edits = ProjectedExample(
            subwords=punc_tokens, labels_star=punc_labels, labels=[]
        )
        non_punc_edits = ProjectedExample(
            subwords=non_punc_tokens, labels_star=non_punc_labels, labels=[]
        )
