...
def main() -> None:
    """Parse arguments and run the two-phase CRF training pipeline."""
    parser = argparse.ArgumentParser(
        description="Train GEC edit‑tagger with optional CRF layer"
    )
    parser.add_argument(
        "--checkpoint",
        default="aubmindlab/bert-base-arabertv02",
        help="Base AraBERT checkpoint identifier",
    )
    parser.add_argument(
        "--train-jsonl",
        default=str(PROCESSED_DATA_DIR / "train_tokens_labels.jsonl"),
        help="Path to training JSONL file",
    )
    parser.add_argument(
        "--label2id", default=str(LABEL2ID_PATH), help="Path to label2id mapping JSON"
    )
    parser.add_argument(
        "--output-dir",
        default=str(
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of training epochs for each phase",
    )
    parser.add_argument(
        "--lr",
        type=float,
        default=5e-5,
        help="Learning rate for full fine‑tuning phase",
    )
    parser.add_argument(
        "--crf-lr",
        type=float,
        default=1e-3,
        help=(
            "Learning rate for CRF-only phase (higher to speed up transition learning)"
        ),
    )
    parser.add_argument(
        "--freeze-bert",
        action="store_true",
        help="Freeze BERT encoder during CRF‑only phase",
    )
    ...
    base_model = load_base_model(
        args.checkpoint, num_labels=num_labels, label2id=label2id
    )
    if args.freeze_bert:
    ...
    logger.info(f"Starting CRF‑only fine‑tuning (BERT frozen = {args.freeze_bert} )")
    trainer_crf.train()
