...
def main():
    ...
    train_texts = []
    for i, text in enumerate(tqdm(train_stream, desc="Caching Train", total=args.train_rows)):
        if i >= args.train_rows: break
    ...
    for config in configs:
        ...
        model_data = smoother.build_model(min_count=config["min_count"], min_n_to_prune=3)
        ...
        if total_words > 0:
            ...
            logger.info(f"Results -> PPL: {perplexity:.2f} (Total Words evaluated: {total_words})")
        else:
