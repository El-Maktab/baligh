...
def main():
    ...
    parser.add_argument("--output", type=str, default="artifacts/nws/evaluation/nwp/report.json")
    ...
    for i, text in enumerate(tqdm(test_stream, desc="Evaluating", total=args.test_rows)):
        if i >= args.test_rows: break
        ...
        for j in range(1, len(tokens)):
            ...
            if target in ['<s>', '</s>', '<unk>'] or target in model.vocab._reverse_punct.values():
                continue
            ...
            if target in predictions:
                ...
                if rank == 1: top_1_hits += 1
                if rank <= 3: top_3_hits += 1
                if rank <= 5: top_5_hits += 1
                mrr_sum += 1.0 / rank
    ...
    metrics = {
        ...
        "top_1_accuracy": (top_1_hits / prediction_events) if prediction_events else 0.0,
        "top_3_accuracy": (top_3_hits / prediction_events) if prediction_events else 0.0,
        "top_5_accuracy": (top_5_hits / prediction_events) if prediction_events else 0.0,
